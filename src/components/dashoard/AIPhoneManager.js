...
const AIPhoneManager = ({ restaurant }) => {
  ...
  const fetchAIPhoneData = async () => {
    try {
      ...
      const [settingsRes, callsRes, commandsRes, numbersRes] = await Promise.all([
      ...
      const [settingsData, callsData, commandsData, numbersData] = await Promise.all([
        settingsRes.json(),
        callsRes.json(),
        commandsRes.json(),
        numbersRes.json()
  ...
  const toggleAIPhone = async () => {
    try {
      ...
      if (data.success) {
        ...
        alert(`AI Phone ${!aiSettings.enabled ? 'enabled' : 'disabled'} successfully!`);
      } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">AI Phone Answering</h2>
          <p className="text-gray-600">Intelligent phone system with voice AI and automated order processing</p>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <div className={`w-16 h-16 rounded-lg flex items-center justify-center ${
              ...
              <Bot className={`w-8 h-8 ${aiSettings.enabled ? 'text-green-600' : 'text-gray-400'}`} />
            </div>
            <div>
              <h3 className="text-xl font-semibold text-gray-900">AI Phone Assistant</h3>
          ...
          {activeCall && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <div className="flex items-center space-x-2 mb-2">
                ...
                <span className="font-medium text-blue-900">Call in Progress</span>
              </div>
              <p className="text-sm text-blue-700">From: {activeCall.callerNumber}</p>
              <p className="text-sm text-blue-700">Duration: {formatDuration(activeCall.duration)}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-gray-600">Answered by AI</p>
              <p className="text-2xl font-bold text-gray-900">{analytics.aiAnswered || 0}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900">{analytics.phoneOrders || 0}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-4">
          ...
          {isListening && (
            <div className="flex items-center space-x-2">
              ...
              <span className="text-blue-600 font-medium">Listening for commands...</span>
            </div>
          )}
        </div>
        ...
        <div className="bg-gray-50 rounded-lg p-6">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <span className="font-semibold">{analytics.answerRate || 0}%</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-600">Average Talk Time</span>
              <span className="font-semibold">{formatDuration(analytics.avgTalkTime || 0)}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-600">Order Conversion</span>
              <span className="font-semibold">{analytics.orderConversion || 0}%</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-600">Customer Satisfaction</span>
              <span className="font-semibold">{analytics.satisfaction || 0}/5</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-50 rounded-lg p-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              ...
              <span className="font-medium">{aiSettings.voiceModel || 'Default'}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Language:</span>
              <span className="font-medium">{aiSettings.language || 'English'}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Max Call Duration:</span>
              <span className="font-medium">{aiSettings.maxDuration || 10} minutes</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Auto Transfer:</span>
              <span className="font-medium">{aiSettings.autoTransfer ? 'Enabled' : 'Disabled'}</span>
            </div>
          </div>
        </div>
      </div>
          ...
          {[1, 2, 3, 4, 5].map((_, index) => (
            <div key={index} className="flex items-center space-x-3 p-3 bg-white rounded-lg">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium">Call answered and order processed</p>
                <p className="text-xs text-gray-500">2 minutes ago</p>
              </div>
              <span className="text-sm font-medium text-green-600">{formatCurrency(25.99)}</span>
            </div>
          ))}
        </div>
      </div>
...
const CallHistoryTab = ({ calls, onSelectCall, getCallStatusColor, getCallStatusIcon }) => {
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold text-gray-900">Call History</h3>
        <span className="text-sm text-gray-600">{calls.length} total calls</span>
      </div>
    ...
    <div className="space-y-4">
          ...
          <div key={call.id} className="bg-gray-50 rounded-lg p-4">
            <div className="flex items-start justify-between mb-3">
              <div>
                ...
                <p className="text-sm text-gray-600">{formatDateTime(call.timestamp)}</p>
                <p className="text-sm text-gray-600">Duration: {formatDuration(call.duration)}</p>
              </div>
              <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                call.orderValue ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
              }`}>
      ...
      {calls.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">No recordings available</h3>
          <p className="mt-1 text-sm text-gray-500">
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <span className="text-sm text-gray-600">{commands.length} commands</span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {commands.map((command, index) => (
          <div key={index} className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="flex items-center justify-between">
              <span className="text-xs text-gray-500">Category: {command.category}</span>
              <span className={`px-2 py-1 rounded-full text-xs font-medium ${
      ...
      {commands.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">No voice commands configured</h3>
          <p className="mt-1 text-sm text-gray-500">
    ...
    <div className="space-y-4">
      ...
      {numbers.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">No phone numbers configured</h3>
          <p className="mt-1 text-sm text-gray-500">
        ...
        <div className="bg-gray-50 rounded-lg p-6">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">{analytics.callsThisWeek || 0}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">This Month</span>
              <span className="font-medium">{analytics.callsThisMonth || 0}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-50 rounded-lg p-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              ...
              <span className="font-medium">{analytics.accuracyRate || 0}%</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Response Time</span>
              <span className="font-medium">{analytics.responseTime || 0}s</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Transfer Rate</span>
              <span className="font-medium">{analytics.transferRate || 0}%</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-50 rounded-lg p-6">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">{formatCurrency(analytics.phoneRevenue || 0)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Avg Order Value</span>
              <span className="font-medium">{formatCurrency(analytics.avgPhoneOrder || 0)}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          <label className="flex items-center">
            ...
            />
            <span className="ml-2 text-sm text-gray-700">Auto-transfer complex orders to staff</span>
          </label>
          ...
          <label className="flex items-center">
            <input
              ...
              onChange={(e) => setFormData({ ...formData, recordCalls: e.target.checked })}
              className="w-4 h-4 text-orange-600 rounded"
            />
            <span className="ml-2 text-sm text-gray-700">Record all calls for quality assurance</span>
          </label>
          ...
          <label className="flex items-center">
            <input
              ...
              onChange={(e) => setFormData({ ...formData, enableTranscription: e.target.checked })}
              className="w-4 h-4 text-orange-600 rounded"
            />
        ...
        <div className="p-6">
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-sm font-medium text-gray-600">Caller</label>
                <p className="font-medium">{call.callerName || call.callerNumber}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-medium">{call.orderValue ? formatCurrency(call.orderValue) : 'No order'}</p>
              </div>
            </div>
