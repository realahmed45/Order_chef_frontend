...
const CustomerExperienceCenter = ({ restaurant }) => {
  ...
  const feedbackCategories = [
    'Food Quality', 'Service', 'Delivery', 'Pricing', 'Ambiance', 'Cleanliness', 'Other'
  ];
  ...
  const campaignTemplates = [
    'Welcome', 'Promotion', 'Birthday', 'Feedback Request', 'Loyalty Reward', 'Event Announcement'
  ];
  ...
  const fetchCustomerData = async () => {
    try {
      ...
      const [reviewsRes, supportRes, chatRes, campaignsRes, feedbackRes, trackingRes, mobileRes] = await Promise.all([
      ...
      const [reviewsData, supportData, chatData, campaignsData, feedbackData, trackingData, mobileData] = await Promise.all([
        reviewsRes.json(),
        supportRes.json(),
        chatRes.json(),
        campaignsRes.json(),
  ...
  const fetchChatMessages = async () => {
    ...
    try {
      const response = await fetch(`/api/customer/chat-messages/${activeChatRoom}`, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  const respondToReview = async (reviewId, response) => {
    try {
      const apiResponse = await fetch(`/api/customer/reviews/${reviewId}/respond`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Customer Experience Center</h2>
          <p className="text-gray-600">Manage reviews, support, feedback, and customer communications</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900">{mobileAppData.totalUsers || 0}</p>
            </div>
          </div>
        </div>
      </div>
...
const CustomerDashboard = ({ 
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Mobile App Performance</h3>
          <div className="space-y-3">
            <div className="flex justify-between">
              <span className="text-gray-600">Total Downloads:</span>
              <span className="font-medium">{mobileAppData.totalDownloads || 0}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Active Users:</span>
              <span className="font-medium">{mobileAppData.activeUsers || 0}</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">{mobileAppData.ordersViaApp || 0}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              ...
              <span className="font-medium">{emailCampaigns.filter(c => c.status === 'active').length}</span>
            </div>
            <div className="flex justify-between">
...
const ReviewsManager = ({ reviews, onRespond, getRatingDistribution, getAverageRating }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-gray-900">Customer Reviews & Ratings</h3>
          ...
          <div className="flex items-center space-x-1">
            {[...Array(5)].map((_, i) => (
              <Star
                key={i}
                className={`w-4 h-4 ${
                  i < Math.round(getAverageRating()) ? 'text-yellow-400 fill-current' : 'text-gray-300'
                }`}
              />
            ))}
          </div>
            ...
            {review.response && (
              <div className="bg-gray-50 rounded-lg p-4 border-l-4 border-orange-400">
                <div className="flex items-center space-x-2 mb-2">
                  ...
                  <span className="text-sm font-medium text-gray-900">Restaurant Response</span>
                </div>
...
const FeedbackForm = ({ feedbackForm, setFeedbackForm, feedbackCategories, supportPriorities }) => {
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => setFeedbackForm({ ...feedbackForm, customerEmail: e.target.value })}
            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500"
          />
        </div>
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => setFeedbackForm({ ...feedbackForm, orderNumber: e.target.value })}
            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500"
