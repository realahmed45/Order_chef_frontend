...
const IntegrationCenter = ({ restaurant }) => {
      ...
      integrations: [
        {
          ...
          features: ['Sales Sync', 'Expense Tracking', 'Tax Reports', 'Inventory Sync'],
          status: 'available'
        },
        {
          id: 'xero',
      ...
      integrations: [
        {
          ...
          features: ['Payment Processing', 'Inventory Sync', 'Customer Data', 'Analytics'],
          status: 'available'
        },
        {
          id: 'clover',
          ...
          features: ['Payments', 'Inventory', 'Reporting', 'Customer Management'],
          status: 'available'
        }
      ]
    },
      ...
      integrations: [
        {
          ...
          features: ['Order Management', 'Menu Sync', 'Analytics', 'Customer Reviews'],
          status: 'available'
        },
        {
          id: 'doordash',
          ...
          features: ['Order Processing', 'Menu Management', 'Promotions', 'Insights'],
          status: 'available'
        },
        {
          id: 'grubhub',
          ...
          features: ['Order Integration', 'Menu Updates', 'Performance Metrics'],
          status: 'available'
        }
      ]
    },
      ...
      integrations: [
        {
          ...
          features: ['Card Payments', 'Digital Wallets', 'Subscriptions', 'Fraud Protection'],
          status: 'available'
        },
        {
          id: 'paypal',
      ...
      integrations: [
        {
          ...
          features: ['Page Management', 'Advertising', 'Analytics', 'Messenger'],
          status: 'available'
        },
        {
          id: 'instagram',
  ...
  const fetchIntegrationData = async () => {
    try {
      ...
      const [integrationsRes, connectedRes, apiKeysRes, webhooksRes, logsRes] = await Promise.all([
      ...
      const [integrationsData, connectedData, apiKeysData, webhooksData, logsData] = await Promise.all([
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Integration Center</h2>
          <p className="text-gray-600">Connect your restaurant with external services and platforms</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900">{connectedApps.length}</p>
            </div>
          </div>
        </div>
          ...
          {activeTab === 'available' && (
            <AvailableIntegrations
              ...
              onConnect={(integration) => {
                ...
                setIntegrationForm({ ...integrationForm, type: integration.id, name: integration.name });
...
const IntegrationDashboard = ({ connectedApps, syncLogs, webhooks, integrationCategories }) => {
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {integrationStats.map((category) => (
            <div key={category.name} className="flex items-center space-x-4 p-4 bg-gray-50 rounded-lg">
          ...
          <div className="space-y-4">
            {recentSyncs.map((sync) => (
              <div key={sync.id} className="flex items-center space-x-4">
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium text-gray-900">{sync.integration}</p>
                  <p className="text-xs text-gray-500">
                    {sync.recordsProcessed} records • {formatDateTime(sync.timestamp)}
                  </p>
                </div>
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                  sync.status === 'success' ? 'bg-green-100 text-green-800' : 
                  sync.status === 'error' ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800'
                }`}>
                  {sync.status}
                </span>
              </div>
...
const AvailableIntegrations = ({ integrationCategories, connectedApps, onConnect }) => {
  return (
        ...
        <div key={category.name} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {category.integrations.map((integration) => {
              const isConnected = connectedApps.some(app => app.type === integration.id);
              
              return (
                <div key={integration.id} className="bg-white border border-gray-200 rounded-lg p-6 hover:border-gray-300 transition">
...
const ConnectedApps = ({ connectedApps, onDisconnect, onSync, getStatusColor, getStatusIcon }) => {
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold text-gray-900">Connected Applications</h3>
        <span className="text-sm text-gray-600">{connectedApps.length} apps connected</span>
      </div>
            ...
            <div key={app.id} className="bg-white border border-gray-200 rounded-lg p-6">
              ...
              <div className="space-y-3 mb-4">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">{formatDateTime(app.lastSync)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">Records Synced:</span>
                  <span className="font-medium">{app.recordsSynced?.toLocaleString() || 0}</span>
                </div>
              </div>
...
const IntegrationSetupForm = ({ integrationForm, setIntegrationForm, selectedIntegration }) => {
  return (
    <div className="space-y-6">
      ...
      <div>
        <label className="flex items-center">
          ...
          />
          <span className="ml-2 text-sm text-gray-700">Enable this integration immediately</span>
        </label>
      </div>
    </div>
  );
      ...
      <div>
        <label className="flex items-center">
          <input
            ...
            className="w-4 h-4 text-orange-600 rounded"
          />
          <span className="ml-2 text-sm text-gray-700">Enable this webhook</span>
        </label>
      </div>
    </div>
  );
