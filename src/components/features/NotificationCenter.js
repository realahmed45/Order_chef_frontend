...
const NotificationCenter = ({ restaurant, currentUser }) => {
  ...
  const fetchNotificationData = async () => {
    try {
      ...
      const [notificationsRes, templatesRes, preferencesRes] = await Promise.all([
  ...
  const handleCreateTemplate = async (e) => {
    ...
    try {
      const url = editingTemplate ? `/api/notifications/templates/${editingTemplate.id}` : '/api/notifications/templates';
  ...
  const handleDeleteTemplate = async () => {
    try {
      const response = await fetch(`/api/notifications/templates/${deleteModal.template.id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Manage all your restaurant communications</p>
        </div>
            ...
            {recentNotifications.map((notification) => (
              <div key={notification.id} className="flex items-center space-x-3 p-3 bg-white rounded-lg">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                  ...
                  <Bell className={`w-4 h-4 ${notification.read ? 'text-gray-600' : 'text-orange-600'}`} />
                </div>
                  ...
                  <div className="flex items-center justify-between text-xs text-gray-500">
                    <div className="flex items-center space-x-2">
                      ...
                      {notification.channels.map((channel) => (
                        <span key={channel} className="px-2 py-1 bg-gray-100 rounded-full">
                          {channel}
                        </span>
                      ))}
                    </div>
    ...
    <div className="space-y-6">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {template.channels.map((channelId) => {
                    ...
                    return (
                      <span key={channelId} className="px-2 py-1 bg-white rounded-full text-xs">
                        {channel?.label}
                      </span>
                    );
                  })}
      ...
      {templates.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">No templates created</h3>
          <p className="mt-1 text-sm text-gray-500">
...
const TemplateForm = ({ templateForm, setTemplateForm, channels, notificationTypes }) => {
  const addVariable = () => {
    setTemplateForm({
      ...templateForm,
              ...
              <input
                ...
                onChange={(e) => {
                  if (e.target.checked) {
                    ...
                    setTemplateForm({
                      ...templateForm,
                      channels: templateForm.channels.filter(c => c !== channel.id)
                    });
                  }
                }}
...
const SendNotificationForm = ({ sendForm, setSendForm, templates, channels }) => {
  return (
    <div className="space-y-6">
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">
              ...
              <input
                ...
                onChange={(e) => {
                  if (e.target.checked) {
                    ...
                    setSendForm({
                      ...sendForm,
                      channels: sendForm.channels.filter(c => c !== channel.id)
                    });
                  }
                }}
                className="w-4 h-4 text-orange-600 rounded"
...
const NotificationPreferences = ({ preferences, onUpdate, channels, notificationTypes }) => {
    ...
    <div className="space-y-8">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              <input
                ...
                onChange={(e) => updateGlobalSettings('quietStart', e.target.value)}
              ...
              <input
                ...
                onChange={(e) => updateGlobalSettings('quietEnd', e.target.value)}
                className="px-2 py-1 border border-gray-300 rounded text-sm"
              />
            </div>
          </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
        <div className="px-6 py-4 bg-gray-50 border-b">
          <h3 className="text-lg font-semibold text-gray-900">Channel Preferences</h3>
          <p className="text-sm text-gray-600">Choose which notifications you want to receive through each channel</p>
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                {channels.map((channel) => (
                  <th key={channel.id} className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase">
                ...
                <tr key={type.value}>
                  ...
                  {channels.map((channel) => (
                    <td key={channel.id} className="px-6 py-4 whitespace-nowrap text-center">
                      <input
                        type="checkbox"
                        checked={preferences.channels?.[channel.id]?.[type.value] !== false}
                        onChange={(e) => updateChannelPreference(channel.id, type.value, e.target.checked)}
                        className="w-4 h-4 text-orange-600 rounded"
                      />
                    </td>
                  ))}
